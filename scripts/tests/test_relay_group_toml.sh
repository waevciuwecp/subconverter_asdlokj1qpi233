...
ext_toml_path="$tmp_dir/external.toml"
...
(
  cd "$tmp_dir"
  "$bin_path" -f "$pref_path" -g --artifact relay_toml_case >/dev/null 2>&1
)
...
echo "PASS: toml relay group behavior is correct"
